...
class GtPredicateBuilderSpec extends Specification {
	...
	def "test Build with all expression arguments"() {
		...
		given: "An Equal PredicateBuilder instance"
			...
			PredicateBuilder predicateBuilder = new GtPredicateBuilder()

		and: "A condition filter with 3 expression arguments"

			ConditionFilter conditionFilter =
					new ConditionFilter(
							ConditionFilterType.GT,
							path("releaseDate"),
							path("releaseDate")
					)
		...
		then: "criteriaBuilder.greaterThan has executed once"

			1 * criteriaBuilder.greaterThan(_,_)
