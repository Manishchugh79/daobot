...
class LtPredicateBuilderSpec extends Specification {
	...
	def "test Build with all expression arguments"() {
		...
		given: "An Equal PredicateBuilder instance"
			...
			PredicateBuilder predicateBuilder = new LtPredicateBuilder()

		and: "A condition filter with 3 expression arguments"

			ConditionFilter conditionFilter =
					new ConditionFilter(
							ConditionFilterType.LT,
							path("releaseDate"),
							path("releaseDate")
					)
		...
		then: "criteriaBuilder.equal has executed once"
			...
			1 * criteriaBuilder.lessThan(_,_)
